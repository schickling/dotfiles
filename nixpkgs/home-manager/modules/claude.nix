{ config, lib, pkgs, ... }:
let
  # Statusline script showing git info, model, context usage, and cost
  # Based on https://github.com/fatih/dotfiles/blob/main/claude/statusline-git.sh
  statuslineScript = pkgs.writeShellScript "claude-statusline-git.sh" ''
    ...
    if [ "$current_usage" != "null" ]; then
      current_tokens=$(echo "$current_usage" | ${pkgs.jq}/bin/jq '.input_tokens + .cache_creation_input_tokens + .cache_read_input_tokens')
    ...
    session_cost=$(echo "$input" | ${pkgs.jq}/bin/jq -r '.cost.total_cost_usd // empty')
    ...
    if ${pkgs.git}/bin/git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
      branch=$(${pkgs.git}/bin/git branch --show-current 2>/dev/null || echo "detached")

      # Get git status with file counts
      status_output=$(${pkgs.git}/bin/git status --porcelain 2>/dev/null)

      if [ -n "$status_output" ]; then
        # Count files and get basic line stats
        total_files=$(echo "$status_output" | wc -l | xargs)
        line_stats=$(${pkgs.git}/bin/git diff --numstat HEAD 2>/dev/null | ${pkgs.gawk}/bin/awk '{added+=$1; removed+=$2} END {print added+0, removed+0}')
  ...
  # Claude Code settings with statusline and existing hooks
  claudeSettings = {
